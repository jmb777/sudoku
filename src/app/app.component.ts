...
export class AppComponent implements OnInit, AfterViewInit {
  ...
  getSavedGrid() {
    ...
    this.killerPuzzle.forEach(cells => {
      cells.forEach(cell => {
        this.gridContents.forEach(gridCell => {
          if (cell.cellNumber === gridCell.cellNumber) {
            const index = this.gridContents.indexOf(gridCell);
            this.gridContents.splice(index, 1, cell);
